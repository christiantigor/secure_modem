...
def decode(fileSrcDecod,fileDecod):
    try:
        print '[DBG]start a3k decode'

        #open file
        f = open(fileSrcDecod,'rb')
        fSrcDecod = f.read()
        f.close()

        #encode file to hex
        h = fSrcDecod.encode('hex')
        ...
        #open decoded unswapped file
        idx = fileDecod.find('.raw')
        fileDecodUnswap = fileDecod[:idx] + '_unswapped' + fileDecod[idx:]
        fDecodUnswap = open(fileDecodUnswap,'wb')

        #split response to chunk
        whole = len(s)
        ...
        for i in range(0,whole,chunkSize):
            ...
            if len(chunk) == chunkSize:
                ...
                chunk = chunk.decode('hex')
                fDecodUnswap.write(chunk)
        fDecodUnswap.close()

        #swap byte
        swap.byteswap(fileDecodUnswap,fileDecod)

        
        print '[DBG]finish a3k decode'
    except:
        print 'a3k decode error'
        sys.exit(1)
